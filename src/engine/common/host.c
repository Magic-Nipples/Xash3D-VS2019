...
void Host_InitCommon( const char *hostname, qboolean bChangeGame )
{
	...
	*out = '\0'; // write terminator
	...
	Con_Reportf( "Sys_LoadLibrary: Loading engine.dll - ok\n" );
...
int EXPORT Host_Main( const char *progname, int bChangeGame, pfnChangeGame func )
{
	...
	host_maxfps = Cvar_Get( "fps_max", "120", FCVAR_ARCHIVE, "host fps upper limit" ); //magic nipples - going to increase to 120 from 72
