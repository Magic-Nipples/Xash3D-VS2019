...
void FS_CreateDefaultGameInfo( const char *filename )
{
	...
	Q_strncpy( defGI.title, "Half-Life", sizeof( defGI.title ));
	...
	Q_strncpy( defGI.game_dll, "bin/server.dll", sizeof( defGI.game_dll ));
	Q_strncpy( defGI.startmap, "c0a0", sizeof( defGI.startmap ));
...
static qboolean FS_ParseLiblistGam( const char *filename, const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_strncpy( GameInfo->title, "Half-Life", sizeof( GameInfo->title ));
