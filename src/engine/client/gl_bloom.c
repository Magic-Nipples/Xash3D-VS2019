...
static void R_Bloom_InitBackUpTexture(int width, int height)
{
	...
	r_bloombackuptexture = GL_CreateTexture("*bloombackuptexture", width, height, NULL, TF_IMAGE);
}
...
static void R_Bloom_InitEffectTexture(void)
{
	...
	if (r_bloom_sample_size->value < 32.0f)
		r_bloom_sample_size->value = 32.0f;
	...
	if (BLOOM_SIZE != r_bloom_sample_size->value)
		r_bloom_sample_size->value = BLOOM_SIZE;

	r_bloomeffecttexture = GL_CreateTexture("*bloomeffecttexture", BLOOM_SIZE, BLOOM_SIZE, NULL, TF_IMAGE);
}
...
static void R_Bloom_InitTextures(void)
{
	...
	if (screen_texture_width > glConfig.max_2d_texture_size || screen_texture_height > glConfig.max_2d_texture_size)
	{
		...
		Con_Printf("'R_InitBloomScreenTexture' too high resolution for light bloom, effect disabled\n");
		return;
	}

	r_bloomscreentexture = GL_CreateTexture("*bloomscreentexture", screen_texture_width, screen_texture_height, NULL, TF_IMAGE);
...
void R_InitBloomTextures(void)
{
	...
	if (r_bloomscreentexture)
		GL_FreeTexture(r_bloomscreentexture);
	if (r_bloomeffecttexture)
		GL_FreeTexture(r_bloomeffecttexture);
	if (r_bloombackuptexture)
		GL_FreeTexture(r_bloombackuptexture);
	if (r_bloomdownsamplingtexture)
		GL_FreeTexture(r_bloomdownsamplingtexture);
